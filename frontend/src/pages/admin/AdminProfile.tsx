...
const AdminProfile = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      setEditForm({ name: res.data.admin.name || '' });
    } catch (err: any) {
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.patch('/admin/profile', editForm);
      setProfileData(res.data.admin);
      if (user && token) {
        login({ ...user, name: res.data.admin.name }, token);
      }
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    setSavingPass(true);
    try {
      await api.patch('/admin/profile/change-password', {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 font-sans">
        <Navbar />
        <LoadingSpinner message="Loading admin profile..." />
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 text-center">
              <div className="w-24 h-24 rounded-full bg-blue-900 text-white flex items-center justify-center text-3xl font-bold mx-auto mb-4 tracking-wider shadow-md">
              ...
              <div className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold bg-blue-100 text-blue-900 border border-blue-200 mb-6">
                <Shield className="w-3.5 h-3.5 mr-1.5" /> Administrator
              </div>
              ...
              <button 
                ...
                className={`w-full py-2.5 px-4 rounded-md text-sm font-bold transition-colors ${isEditMode ? 'bg-slate-100 text-slate-700 border border-slate-300 hover:bg-slate-200' : 'bg-blue-900 text-white hover:bg-blue-800 shadow-sm border border-transparent'}`}
              >
            ...
            <div className="bg-white shadow-sm border border-slate-200 rounded-xl overflow-hidden">
              <div className="bg-white border-b border-slate-100 px-5 py-4 flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-slate-900">System Statistics</h3>
              </div>
              <div className="px-5 py-4 space-y-4">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-600 flex items-center"><UserIcon className="w-4 h-4 mr-1.5 text-blue-600"/> Providers Registered</span>
                  <span className="font-bold text-slate-900 bg-slate-100 px-2 py-0.5 rounded-md">{stats?.total_providers || 0}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-600 flex items-center"><FileText className="w-4 h-4 mr-1.5 text-blue-600"/> Total Requests</span>
                  <span className="font-bold text-slate-900 bg-slate-100 px-2 py-0.5 rounded-md">{stats?.total_requests || 0}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-600 flex items-center"><Clock className="w-4 h-4 mr-1.5 text-yellow-500"/> Pending Review</span>
                  <span className="font-semibold bg-yellow-50 text-yellow-700 px-2 py-0.5 rounded-full border border-yellow-200">{stats?.pending_review || 0}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-600 flex items-center"><FileText className="w-4 h-4 mr-1.5 text-emerald-500"/> Documents Uploaded</span>
                  <span className="font-semibold bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full border border-emerald-200">{stats?.documents_uploaded || 0}</span>
                </div>
              </div>
              <div className="bg-slate-50 border-t border-slate-100 p-3 text-center">
                <Link to="/admin/dashboard" className="text-sm text-blue-900 hover:text-blue-700 font-bold hover:underline inline-flex items-center">
                  View Admin Dashboard <span className="ml-1 text-lg mb-0.5">&rarr;</span>
                </Link>
              </div>
            </div>
          </div>
              ...
              <div className="p-6">
                {!isEditMode ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-8 gap-x-8">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-500 mb-1 uppercase tracking-wider">Role</p>
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-sm font-bold bg-blue-100 text-blue-900 border border-blue-200 mt-1">
                        Administrator
                      </span>
                    </div>
                    <div>
                  ...
                  <form onSubmit={handleProfileSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div>
                        ...
                        <input 
                          ...
                          className="w-full px-4 py-2 bg-white border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-900 focus:border-blue-900 sm:text-sm text-slate-900 font-medium"
                        />
                      </div>
                      <div>
                    ...
                    <div className="pt-5 flex justify-end gap-3 border-t border-slate-100">
                      ...
                      <button 
                        ...
                        className="px-5 py-2 bg-blue-900 text-white border border-transparent rounded-md shadow-sm text-sm font-bold hover:bg-blue-800 transition-colors disabled:opacity-50 flex items-center gap-2"
                      >
                ...
                <form onSubmit={handlePasswordSubmit} className="space-y-6 max-w-lg">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-900 focus:border-blue-900 sm:text-sm text-slate-900 font-medium"
                    />
                  </div>
                  <div>
                  ...
                  <div className="pt-2">
                    <button 
                      ...
                      className="px-5 py-2.5 bg-blue-900 text-white border border-transparent rounded-md shadow-sm text-sm font-bold hover:bg-blue-800 transition-colors disabled:opacity-50"
                    >
